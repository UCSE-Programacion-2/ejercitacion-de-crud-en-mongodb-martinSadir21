...
app.get('/equipos/buscar', async (req, res) => {
    try {
        ...
        // Si no mandan el parámetro, devolvemos un error 400 o un arreglo vacío según lo que espere el test. 
        // Por las dudas, si falta, filtramos por un string vacío que trae todo o manejamos el caso:
        const terminoBusqueda = tecnico || '';

        const filtro = { tecnico: { $regex: terminoBusqueda, $options: 'i' } };
        const equiposFiltrados = await req.collection.find(filtro).toArray();
        
        res.status(200).json(equiposFiltrados);
    } catch (error) {
        res.status(500).json({ error: "Error en la búsqueda del técnico" });
    }
});
...
app.get('/equipos/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Error al buscar el equipo por ID" });
    }
});
...
app.post('/equipos', async (req, res) => {
    try {
        ...
        res.status(201).json({ _id: resultado.insertedId, ...nuevoEquipo });
    } catch (error) {
...
app.put('/equipos/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ mensaje: "Equipo actualizado" });
    } catch (error) {
